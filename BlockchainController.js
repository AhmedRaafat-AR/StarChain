...
class BlockchainController {
    ...
    // This endpoint allows you to retrieve the blockchain status 
    validateChain(){
        this.app.get("/validateChain", async(req, res) =>{        
            let errLength = await this.blockchain.validateChain();
            if(errLength.length === 0){
                return res.status(200).send("BlockChain is Healthy");
            }else{
                return res.status(500).send("Blockchain is Broken");
            }
        });
    }
